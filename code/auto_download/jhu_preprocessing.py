...
def preprocess_by_country(country_names=['Germany'], fips_codes=['GM']):
    ...
    for country, code in zip(country_names, fips_codes):
        ...
        df.to_csv('../../data-truth/JHU/truth_JHU-Cumulative Deaths_{}.csv'.format(country), index=False)
        
        # compute incident deaths
        df_inc = df.copy()
        ...
        df_inc.to_csv('../../data-truth/JHU/truth_JHU-Incident Deaths_{}.csv'.format(country), index=False)
