...
make_qntl_dat <- function(path,all_states=FALSE) {
  ...
  col_list1 <- c(grep("location_name", colnames(data)),grep("date", colnames(data)),grep("death", colnames(data)))
  death_qntl1 <- data[,col_list1] %>% #only take important rows
    ...
    tidyr::pivot_longer(-c(location_name, date_v, target_id),names_to = "quantile",values_to = "value")%>% 
  ...
  col_list2 <- c(grep("location_name", colnames(data)),grep("date", colnames(data)),grep("totdea", colnames(data)))
  ...
  death_qntl2_2 <- death_qntl2_1 %>%
    dplyr::rename("0.025"=totdea_lower,"0.975"=totdea_upper,"NA"=totdea_mean) %>%
    tidyr::pivot_longer(-c(location_name, date_v,day_v, ew, target_id),names_to = "quantile",values_to = "value")%>% 
  ...
  comb <-rbind(death_qntl1,death_qntl2,death_qntl2_2) #deleted death_qntl2_2, needs to be added later
  
  #only take german data (i.e. the ones recognized in state_codes.csv)
  comb <- comb %>%
  dplyr::filter(!is.na(location)) %>%
  #parse to make sure format is right
  dplyr::mutate(target_end_date=as.Date(target_end_date),quantile=as.numeric(quantile),value=as.numeric(value))
  #get final data frame, i.e. order
  final<- comb %>%
    #make sure to not select any extra columns
    dplyr::select(forecast_date,target_id,target_end_date,location,location_name,type,quantile,value) %>%
    #rename last columns
